...
Plane::Plane(Graphics& gfx, float xPos, float yPos, float zPos, std::string surfacePath)
	:
	xPos(xPos),
	yPos(yPos),
	zPos(zPos),
...
void Plane::UpdateMatrices(Graphics & gfx)
{
...
void Plane::SetPosition(float _x, float _y, float _z)
{
	xPos = _x;
	yPos = _y;
	zPos = _z;
...
// Sets in radians
void Plane::SetRotation(float _pitch, float _yaw, float _roll)
{
	pitch = _pitch;
	yaw = _yaw;
	roll = _roll;
...
void Plane::SetScale(float _x, float _y, float _z)
{
	xScale = _x;
	yScale = _y;
	zScale = _z;
